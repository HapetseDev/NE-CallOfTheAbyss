...
const COMBATANT_SCENE := preload("res://Scenes/Battle/battle_combatant.tscn")
...
func _setup_encounter() -> void:
	var encounter := BattleManager.pending_encounter
	if encounter == null:
	...
	for i in BattleManager.party_battle_resources.size():
		var res := BattleManager.party_battle_resources[i].duplicate(true) as BattleCharacterResource
		if res == null:
			continue
...
func _spawn_combatant(
	...
	add_child(combatant)
	combatant.global_position = position
