...
func _refresh() -> void:
	...
	var sheet := _selected.character_sheet
	if sheet == null:
		_summary_label.text = "Kein Charakterbogen vorhanden."
		return

	_title_label.text = "Debug: %s" % sheet.character_name
	...
	for section in sheet.get_attribute_sections():
		_attributes_box.add_child(_build_attribute_editor(section, sheet))


func _ensure_sheet(playable: Playable) -> void:
	if playable.character_sheet != null:
		return
	playable.character_sheet = _SheetFactory.create_default(playable.get_display_name())
	playable.character_sheet.ensure_initialized()


func _build_attribute_editor(section: Dictionary, sheet: CharacterSheet) -> Control:
	...
	base_spin.value = sheet.get_base_attribute(attr)
	base_spin.value_changed.connect(_on_base_attribute_changed.bind(sheet, attr))
	...
	var learned := sheet.get_learned_skills_for_attribute(attr)
	for learned_skill in learned:
		vbox.add_child(_build_skill_row(sheet, learned_skill))
	...
	var unlearned: Array[SkillDefinition] = sheet.get_unlearned_talents_for_attribute(attr)
	...
	if not add_select.disabled:
		add_btn.pressed.connect(_on_add_talent_pressed.bind(sheet, attr, add_select, unlearned))
	add_row.add_child(add_btn)

	return panel


func _build_skill_row(sheet: CharacterSheet, learned: LearnedSkill) -> Control:
	...
	level_spin.max_value = sheet.get_skill_level_cap(learned.skill_id)
	level_spin.value = learned.level
	level_spin.value_changed.connect(_on_skill_level_changed.bind(sheet, learned.skill_id))
	row.add_child(level_spin)

	var remove_btn := Button.new()
	remove_btn.text = "−"
	remove_btn.pressed.connect(_on_remove_skill_pressed.bind(sheet, learned.skill_id))
	row.add_child(remove_btn)

	return row


func _on_base_attribute_changed(sheet: CharacterSheet, attr: CharacterEnums.Attribute, value: float) -> void:
	sheet.debug_set_base_attribute(attr, int(value))
	_refresh()


func _on_skill_level_changed(sheet: CharacterSheet, skill_id: String, value: float) -> void:
	sheet.debug_set_skill_level(skill_id, int(value))
	_refresh()


func _on_remove_skill_pressed(sheet: CharacterSheet, skill_id: String) -> void:
	sheet.debug_remove_skill(skill_id)
	_refresh()
...
func _on_add_talent_pressed(
	...
	if sheet.debug_add_skill(definition.skill_id, 1):
		_refresh()
